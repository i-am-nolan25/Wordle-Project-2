...
class BoardController: NSObject,
  ...
  private func applyNumLettersSettings(with settings: [String: Any]) {
    ...
    if let numLetters = settings[kNumLettersKey] as? Int {
        // Check if numLetters is within the allowed range
        let clampedNumLetters = max(kMinLetters, min(kMaxLetters, numLetters))
        // Assign the clampedNumLetters to the numItemsPerRow property
        numItemsPerRow = clampedNumLetters
    }
    // END YOUR CODE HERE
  }
  ...
  private func applyNumGuessesSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      if let numGuesses = settings[kNumGuessesKey] as? Int {
          // Check if numGuesses is within the allowed range
          let clampedNumGuesses = max(kMinGuesses, min(kMaxGuesses, numGuesses))
          // Assign the clampedNumGuesses to the numRows property
          numRows = clampedNumGuesses
      }
    // END YOUR CODE HERE
  }
  ...
  private func applyIsAlienWordleSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      if let isAlienWordle = settings[kIsAlienWordleKey] as? Bool {
          // Assign the value of isAlienWordle to the property isAlienWordle
